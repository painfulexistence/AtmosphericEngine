...
inline CSG::NodePtr SimpleRoom(
    ...
    if (openFront) {
        ...
        auto door = CSG::Box(
            center + glm::vec3(0, -size.y/2 + doorHeight/2, size.z/2),
            doorSize
        );
    ...
    if (openLeft) {
        ...
        auto door = CSG::Box(
            center + glm::vec3(-size.x/2, -size.y/2 + doorHeight/2, 0),
            doorSize
        );
        room = CSG::Subtract(room, door);
    }
...
inline CSG::NodePtr Platform(
    glm::vec3 center,
    glm::vec3 size,
    int stairSteps = 0,
    float stairWidth = 2.0f
) {
...
inline CSG::NodePtr WindowCutout(
    ...
    return CSG::Box(
        wallCenter + glm::vec3(0, sillHeight + windowHeight/2, 0),
        windowSize
    );
}
...
inline CSG::NodePtr ExampleDungeonRoom() {
    return SimpleRoom(
        ...
        0.5f,                      // wall thickness
        true,                      // open front
        false, false, false        // closed other sides
    );
}

// L-shaped corridor
inline CSG::NodePtr ExampleLCorridor() {
    auto corridor1 = Corridor(
        glm::vec3(0, 0, 0),
        glm::vec3(10, 0, 0),
        3.0f, 3.0f
    );

    auto corridor2 = Corridor(
        glm::vec3(10, 0, 0),
        glm::vec3(10, 0, 10),
        3.0f, 3.0f
    );
...
inline CSG::NodePtr ExampleDungeonLayout() {
    ...
    auto entryRoom = SimpleRoom(
        ...
        glm::vec3(8, 5, 8),
        0.4f,
        false, true, false, true  // open back and right
    );

    // Corridor to main hall
    auto corridor = Corridor(
        glm::vec3(0, 1.5f, -4),
        glm::vec3(0, 1.5f, -12),
        3.0f, 3.0f
    );
    ...
    auto mainHall = SimpleRoom(
        ...
        glm::vec3(15, 6, 12),
        0.5f,
        true, false, true, true  // open front, left, right
    );
    ...
    auto sideRoom = SimpleRoom(
        ...
        glm::vec3(6, 5, 6),
        0.4f,
        false, false, true, false  // open left
    );
