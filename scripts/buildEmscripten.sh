...
echo -e "${BLUE}===================================================${NC}"
echo -e "${YELLOW}🎮 Atmospheric Engine - Emscripten 建置指令碼${NC}"
echo -e "${BLUE}===================================================${NC}"
...
# 5. 進行建置 (目標包含 AtmosLua 與 HelloWorld，這兩個是支援網頁建置的目標)
echo -e ""
echo -e "${YELLOW}🔨 正在使用 Emscripten 建置 AtmosLua 與 HelloWorld...${NC}"
cmake --build "$BUILD_DIR" --target AtmosLua HelloWorld --parallel

echo -e ""
echo -e "${GREEN}✨ WebAssembly / Emscripten 建置成功！${NC}"
echo -e "網頁版產物已輸出至："
...
if [ "$RUN_SERVER" = "y" ] || [ "$RUN_SERVER" = "Y" ]; then
    ...
    # 如果是 Mac，自動幫忙在瀏覽器中打開 AtmosLua 網頁
    if [[ "$OSTYPE" == "darwin"* ]]; then
        sleep 1 && open "http://localhost:$PORT/AtmosLua/AtmosLua.html" &
    fi
