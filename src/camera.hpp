...
class Camera {
    public:
        ...
        void backAndForthMove(float sign, float dt) {
            ...
            glm::vec3 velocity = sign * direction_cos * SPEED;
        ...
        void horizontallyMove(float sign, float dt) {
            glm::vec3 direction = glm::vec3(glm::cos(_hAngle) * glm::cos(_vAngle), glm::sin(_vAngle), glm::sin(_hAngle) * glm::cos(_vAngle));
        ...
        void yaw(float sign, float dt) {
            _hAngle += sign * dt * ANGULAR_SPEED;
        }

        void pitch(float sign, float dt) {
            _vAngle = std::max(minVAngle, std::min(maxVAngle, _vAngle + sign * dt * ANGULAR_SPEED));
        }
